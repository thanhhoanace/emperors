...
(function () {
  ...
  F.trees = function (terr, o) {
    ...
    const place = (x, z, f, y0) => {
      ...
      const s = rr(0.8, 1.2) * (0.75 + 0.35 * f) * PS;
      ...
      (conifer ? lists.pine : lists.broad).push({ m: new THREE.Matrix4().compose(new THREE.Vector3(x, h - 0.05 * PS, z), new THREE.Quaternion().setFromAxisAngle(_y, rnd() * 6.28), new THREE.Vector3(s, s * rr(0.9, 1.25), s)), c: tint.clone() });
    };
    if (o.R) {
      for (let x = o.cx - o.R; x < o.cx + o.R; x += sp) for (let z = o.cz - o.R; z < o.cz + o.R; z += sp) {
        const jx = x + rr(-0.4, 0.4) * sp, jz = z + rr(-0.4, 0.4) * sp; if (Math.hypot(jx - o.cx, jz - o.cz) > o.R) continue;
        const f = terr.forestAt(jx, jz); if (f < (o.minForest ?? 0) || rnd() > f * 1.1) continue;
        place(jx, jz, f);
      }
    }
    for (const [x, z, f, y] of o.extra || []) place(x, z, f ?? 0.8, y); // e.g. palace gardens inside city walls
    // lone trees: along field edges, roads and villages (TW3K scatters them everywhere)
    const G = terr.G;
    for (let k = 0, n = 0; k < 300000 && n < (o.lone ?? 5000); k++) {
      const x = o.loneR ? o.cx + rr(-o.loneR, o.loneR) : rr(G.x0 + 2, G.x0 + G.w - 2), z = o.loneR ? o.cz + rr(-o.loneR, o.loneR) : rr(G.z0 + 2, G.z0 + G.d - 2);
      if (!o.loneR && o.R && Math.hypot(x - o.cx, z - o.cz) < o.R) continue;
      const f = terr.forestAt(x, z), fl = terr.fieldAt(x, z), rd = terr.roadD(x, z);
      if (f > 0.1 || terr.h(x, z) < (o.minH ?? 0.45) || terr.riverSD(x, z) < 0.8 * PS || rd < 0.6 * PS || (o.avoid && o.avoid(x, z))) continue;
  ...
  F.hamlets = function (terr, o = {}) {
    ...
    for (const hm of terr.hamlets) {
      ...
      for (let k = 0; k < hm.n; k++) {
        const a = rnd() * 6.28, r = rr(0.3, 2.2) * Math.sqrt(PS), x = hm.x + Math.cos(a) * r, z = hm.z + Math.sin(a) * r;
        if (terr.riverSD(x, z) < 0.6 * PS) continue;
        (rnd() < 0.5 ? LA : LB).push({ m: new THREE.Matrix4().compose(new THREE.Vector3(x, terr.h(x, z) - 0.02 * PS, z), new THREE.Quaternion().setFromAxisAngle(_y, yaw0 + (rnd() < 0.3 ? Math.PI / 2 : 0)), new THREE.Vector3(PS, PS, PS)) });
      }
    }
    return [...F.chunk(houseA, houseMat, LA, 80), ...F.chunk(houseB, houseMat, LB, 80)];
  };

  // The Great Wall: round 4 follows the Yan ridge; round 5 the Han wall from lon/lat (terr.wall), rammed loess
  // with beacon towers (no brick in the Han).
  F.greatWall = function (terr, o = {}) {
    const PS = o.scale ?? 1;
    const line = Terrain.spline(terr.wall && terr.wall.length ? terr.wall : Terrain.geo.WALL.map(([x, z]) => [x * terr.S, z * terr.S]), 0.25 * (o.step ?? 1));
    const parts = [];
    const wallMat = Terrain.receiveBaked(new THREE.MeshStandardMaterial({ color: o.color ?? 0x8b8173, roughness: 0.95 }), terr);
    for (let i = 0; i < line.length - 1; i++) {
      ...
      if (terr.riverSD && terr.riverSD(x, z) < 0.3) continue; // no wall across rivers
      parts.push(new THREE.BoxGeometry(0.34 * PS, 0.5 * PS, len + 0.04).applyMatrix4(new THREE.Matrix4().compose(new THREE.Vector3(x, y + 0.1 * PS, z), new THREE.Quaternion().setFromAxisAngle(_y, yaw), new THREE.Vector3(1, 1, 1))));
      if (i % (o.towerEvery ?? 14) === 0) parts.push(new THREE.BoxGeometry(0.62 * PS, 0.95 * PS, 0.62 * PS).translate(x, y + 0.3 * PS, z));
    }
  ...
  // Wooden bridges where roads cross water. o.wet(x, z) marks water (default: below the round-4 sea line);
  // o.deck(a, b) gives the deck height (default: the round-4 water plane).
  F.bridges = function (terr, o = {}) {
    const wet = o.wet || ((x, z) => terr.h(x, z) < 0.25), PS = o.scale ?? 1;
    const parts = [];
    for (const r of terr.roads) {
      ...
      const flush = () => {
        if (run.length > 1) {
          ...
          if (o.maxLen && len > o.maxLen) { run = []; return; } // wide rivers were crossed by ferry
          const y = o.deck ? o.deck(a2, b2) : 0.42;
          const m = new THREE.Matrix4().compose(new THREE.Vector3((a2[0] + b2[0]) / 2, y, (a2[1] + b2[1]) / 2), new THREE.Quaternion().setFromAxisAngle(_y, yaw), new THREE.Vector3(1, 1, 1));
          parts.push(new THREE.BoxGeometry(0.55 * PS, 0.08 * PS, len).applyMatrix4(m));
          for (let t = -len / 2; t <= len / 2; t += 0.9 * PS) for (const e of [-0.24 * PS, 0.24 * PS]) parts.push(new THREE.BoxGeometry(0.06 * PS, 0.8 * PS, 0.06 * PS).translate(e, 0.05 * PS, t).applyMatrix4(m));
        }
        run = [];
      };
      for (let k = 0; k < r.pts.length; k++) { const [x, z] = r.pts[k]; if (wet(x, z)) run.push([x, z]); else flush(); }
      flush();
    }
  ...
  F.clouds = function (list, o = {}) {
    ...
    // fade to nothing at the sprite edge, or the puffs clipped by the canvas show as a rectangle
    g.globalCompositeOperation = 'destination-in';
    const edgeFade = g.createRadialGradient(128, 128, 40, 128, 128, 128); edgeFade.addColorStop(0, 'rgba(0,0,0,1)'); edgeFade.addColorStop(1, 'rgba(0,0,0,0)');
    g.fillStyle = edgeFade; g.fillRect(0, 0, 256, 256);
